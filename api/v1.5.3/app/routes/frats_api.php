...
MÉTODO: POST
REQUERIMIENTOS: TO-DO identificar el req actualización de usuarios
TESTS: api/frats_update_post.sh
...
SQLS: 	 autogenerado
--*/
$app->post('/frats/update', function () use($app) {

	try{
		$authorized = checkPerm('POST:/frats/update', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$tableName = 'fraternidades';
		      ...
		      $connection = null;
				...
				$connection = null;
				$app->response->body("/frats/update (POST) " . ACCESSERROR);
...
SQLS: 	 autogenerado
--*/
$app->post('/frats/add', function () use($app) {

	try{
		$authorized = checkPerm('POST:/frats/add', $app);
		if($authorized){
				$resultText = checkToken($app);
				if(contains("validtoken", $resultText) ){
					$tableName = 'fraternidades';
		      ...
		      $connection = null;
				...
				$connection = null;
				$app->response->body("/frats/add (POST) " . ACCESSERROR);
